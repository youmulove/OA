...
public class ApprovalRuleController extends BaseController {
	...
	public JsonResult deleteApprovalRule(String aa, String approvalRole,
			ModelMap map, String isOver, String approvalRank) {
		String message = "设置审批删除失败，请重新操作！";
		String flag = "0";
		ApprovalRule rule = new ApprovalRule();
		rule.setApprovalType(aa);

		if (StringUtils.isNotBlank(aa)
				&& StringUtils.isNotBlank(UserUtils.getUser().getCompany()
						.getId())) {
			rule.setApprovalCompany(UserUtils.getUser().getCompany().getId());
			approvalRuleService.deleteByApprovalRule(rule);
			message = "设置审批删除成功！";
		...
		return new JsonResult(flag, message, aa);
	}
