...
<body>
      ...
      <aside class="m__aside j-aside">
        <ul>
          <c:if test="${type=='leave'}">
	          ...
	          <li data-url="url_currency" onclick="myProcess('currency')"><img src="${ctxStatic}/approvalProcess/img/common.png" alt=""/>通用</li>
	        </c:if>
	        ...
	        <c:if test="${type=='currency'}">
	          ...
	          <li data-url="url_home" onclick="myProcess('officeRoom')"><img src="${ctxStatic}/approvalProcess/img/home.png" alt=""/>会议室</li>
	          <li data-url="url_projector" onclick="myProcess('projector')"><img src="${ctxStatic}/approvalProcess/img/projector.png" alt=""/>投影仪</li>
	          <li data-url="url_currency" class="hover" onclick="myProcess('currency')"><img src="${ctxStatic}/approvalProcess/img/common.png" alt=""/>通用</li>
	        </c:if>
      </aside>
            ...
            <table class="c-table">
	        	...
	        	<c:forEach items="${list}" var="rule" varStatus="st">
		        	<tr>
		        		...
		        		&nbsp;&nbsp;&nbsp;<a onclick="deleteApprovalRule('${type}','${rule.approvalRole}','${rule.approvalRank}')" class="m--btn">删除</a>
		        		</td>
		        	</tr>
	        	</c:forEach>
	        	</tbody> 
  ...
  <script>
  	...
  	function deleteApprovalRule(aa,approvalRole,approvalRank){
  		layer.confirm('&nbsp;&nbsp;&nbsp;&nbsp;确定要删除该审批？', {
		  btn: ['确定', '取消'] //可以无限个按钮
		}, function(index, layero){
		  $.ajax({
  			url:"${ctx}/oa/approvalRule/deleteApprovalRule",
  			data:{"aa":aa,"approvalRole":approvalRole,"approvalRank":approvalRank},
  			dataType:"json",
  			type:"post",
  			success:function(data){
  				if(data.state=='0'){
  					window.location.href = "${ctx}/oa/approvalRule/alreadyProcess?type="+data.data;
  				}else{
