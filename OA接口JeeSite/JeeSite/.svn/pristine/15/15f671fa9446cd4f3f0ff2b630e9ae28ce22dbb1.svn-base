...
public class SystemService extends BaseService implements InitializingBean {
	...
	public String getParentsNames(User user) {
		...
		// System.out.println(fir + office.getName());
	...
	public String getOfficeByCompany(String commpanyName) {
		if (StringUtils.isNotBlank(commpanyName)) {
			String remarks = officeDao.getOfficeByCompany(commpanyName)
					.getRemarks();
			return remarks;
		}
		return null;
	...
	public List<User> userList(User user) {
		...
		for (User u : users) {
			u = UserUtils.get(u.getId());
			u.setOfficeNames(this.getParentsNames(u));
			userRoles.add(u);
		}
		return userRoles;
	}

	public Page<User> findWithRoles2(Page<User> page, User user) {
		user.getSqlMap().put("dsf",
				dataScopeFilter(user.getCurrentUser(), "o", "a"));
		List<User> users = userDao.findList(user);
		List<User> userRoles = new ArrayList<User>();
	...
	public Page<User> findWithRoles(Page<User> page, User user) {
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		/*
		 * user.getSqlMap().put("dsf", dataScopeFilter(user.getCurrentUser(),
		 * "o", "a")); List<User> users = userDao.findList(user); List<User>
		 * userRoles = new ArrayList<User>();
		 * 
		 * for (User u : users) { u = UserUtils.get(u.getId());
		 * u.setOfficeNames(this.getParentsNames(u)); userRoles.add(u); }
		 */
		user.setPage(page);
		page.setList(this.userList(user));
		return page;
	...
	public List<User> userList1(User user) {
		 ...
		 * "o", "a"));
		 */
		List<User> users = userDao.findList1(user);
		List<User> userRoles = new ArrayList<User>();
		for (User u : users) {
			u = UserUtils.get(u.getId());
			u.setOfficeNames(this.getParentsNames(u));
	...
	public Page<User> findWithRoles1(Page<User> page, User user) {
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		/*
		 * user.getSqlMap().put("dsf", dataScopeFilter(user.getCurrentUser(),
		 * "o", "a")); List<User> users = userDao.findList(user); List<User>
		 ...
		 */
		user.setPage(page);
		page.setList(this.userList1(user));
		return page;
	...
	/**
	 * 通过公司ID获取职级列表，仅返回职级id和name（树查询职级时用）
	 * 
	 * @param officeId
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public List<SysRank> findsysRankByOfficeId(String officeId) {
		List<SysRank> list = (List<SysRank>) CacheUtils.get(
				UserUtils.SYSRANK_CACHE,
				UserUtils.SYSRANK_CACHE_LIST_BY_COMPANY_ID_ + officeId);
		if (list == null) {
			SysRank sysRank = new SysRank();
			sysRank.setCompany(new Office(officeId));
			list = sysrankDao.findsysRankByOfficeId(sysRank);
			CacheUtils.put(UserUtils.SYSRANK_CACHE,
					UserUtils.SYSRANK_CACHE_LIST_BY_COMPANY_ID_ + officeId,
					list);
		}
		return list;
	...
	/**
	 * 通过职级ID获取用户列表，仅返回用户id和name（树查询用户时用）
	 * 
	 * @param user
	 * @return
	 */
	public List<User> findUserBysystrankId(String sysrankId) {
		List<User> list = new ArrayList<User>();
		/*
		 * (List<User>) CacheUtils.get(UserUtils.USER_CACHE,
		 * UserUtils.USER_CACHE_LIST_BY_SYSRANK_ID_ + sysrankId);
		 */
		if (list.size() == 0) {
			list = userDao.findUserBysysrankId(sysrankId, UserUtils.getUser()
					.getCompany().getId());
			CacheUtils.put(UserUtils.USER_CACHE,
					UserUtils.USER_CACHE_LIST_BY_SYSRANK_ID_ + sysrankId, list);
		}
		return list;
	}
	...
	@Transactional(readOnly = false)
	public void updateEmailById(String id, String loginName, String newEmail) {
		User user = new User(id);
		user.setEmail(newEmail);
		userDao.updateEamilById(user);
		// 清除用户缓存
		user.setLoginName(loginName);
		UserUtils.clearCache(user);
		// // 清除权限缓存
	...
	public List<User> findUserRoles() {
		List<User> userId = userDao.findUserId();
		List<User> userRoleList = UserUtils.getRoles(userId);
		return userRoleList;
	}
	// /////////////// Synchronized to the Activiti end //////////////////
		//更新用户信息    
		 	@Transactional(readOnly = false)
			public User updateUserInfo1(User user) {
				user.preUpdate();
				userDao.updateUserInfo(user);
				return user;
			}
	...
	// /////////////// Synchronized to the Activiti end //////////////////
		 // 根据用户id修改用户性别
	    	@Transactional(readOnly = false)
	    	public void updateMobileById(String loginName, String mobile) {
	    		User user = UserUtils.getByLoginName(loginName);
	    		user.setMobile(mobile);
	    		userDao.updateMobileById(user);
	    		// 清除用户缓存
	    		user.setLoginName(loginName);
	    		UserUtils.clearCache(user);
	    	}
	    	// 根据用户id修改用户的消息接受状态
	    	@Transactional(readOnly = false)
	    	public void updateStateById(String loginName, String state) {
	    		User user = UserUtils.getByLoginName(loginName);
	    		user.setState(state);
	    	...
	    	// 根据用户id获取用户目前接受消息的状态
	    	@Transactional(readOnly = false)
	    	public String getStateByid(String loginName) {
	    		User user = UserUtils.getByLoginName(loginName);
	    		return userDao.getStateByid(user);
	    	}
}
