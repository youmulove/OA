...
public class ProjectorService extends BaseService {
	@Autowired
	 ...
	 * 投影仪申请
	 */
	public void projectorApply(OaApplyProjector projector,
			String oaNotifyRecordIds, String projectoroNo, String sysapp1,
			String sysapp2, String sysapp3, String sysapp4) {
		// 设置查询条件 本公司
		...
		if(appList.get(0).getApprovalProcess().equals("0")){
			if(sysapp1!=null&&!sysapp1.equals("")){
				str1=sysapp1;
				projector.setAuditFirId(sysapp1);
				processDefinitionKey="1";	
			}
	       if(sysapp2!=null&&!sysapp2.equals("")){
	    	   str2=sysapp2;
	    	   projector.setAuditSecId(sysapp2);
	    	   processDefinitionKey="2";
				}
	       if(sysapp3!=null&&!sysapp3.equals("")){
	    	   str3=sysapp3;
	    	   projector.setAuditThrId(sysapp3);
	    	   processDefinitionKey="3";
	        }
				
	       if(sysapp4!=null&&!sysapp4.equals("")){
	    	   str4=sysapp4;
	    	   projector.setAuditFouId(sysapp4);
			...
			if (app.getApprovalRole().equals("1")) {
				...
				projector.setAuditFirId(str1);// 设置第一审批人
				...
				projector.setAuditSecId(str2);// 第二审批人
				...
				projector.setAuditThrId(str3);// 第三审批人
				...
				projector.setAuditFouId(str4);
			}
		}
			}
		projector.setId(UUID.randomUUID().toString().replace("-", ""));
		projector.setCreateBy(user);
		projector.setCreateDate(new Date());
		projector.setCopytoId(oaNotifyRecordIds);
		projector.setApplyTime(new Date());
		projector.setUpdateBy(user);
		projector.setUpdateDate(new Date());
		projector.setCompany(UserUtils.getUser().getCompany());
		// projector.setOffice(UserUtils.getUser().getOffice());
		projector.setOffice(officeService.findAllDeptByUser(user));
		projector.setApplyUserid(user.getName());

		HashMap<String, Object> variables = new HashMap<String, Object>();
		variables.put("type", "projector");
		...
		projector.setProcessInstanceId(processInstance.getId());
		// 存入申请人角色信息
		...
		variables.put("projector", projector);
		...
		variables.put("title", user.getName() + "的投影仪申请");
		...
		projector.setIsNewRecord(true);
		projector.setAdapterId(projectoroNo);
		projector.setYuliu1(oaProjectorService.get(projectoroNo)
				.getProjectorName());// 往申请表存入投影仪名称
		OaApplyProjectorDao.insert(projector);
	}
