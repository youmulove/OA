...
<body>
		...
		<table class="c-table">
			...
			<c:if test="${type=='leave'}">
				...
				<tr><td>请假时间：</td>
					...
					~<fmt:formatDate value="${leave.endTime}" type="both"/></td>
				</tr>
				<tr><td>请假天数：</td>
					<td>${leave.hours}</td>
				</tr>
			...
			<c:if test="${type=='overTime'}">
				...
				<tr><td>加班时间：</td>
					...
					~<fmt:formatDate value="${overTime.endTime}" type="both"/></td>
				</tr>
				<tr><td>加班时长：</td>
				...
				<tr><td>是否是节假日：</td>
					<td><c:if test="${overTime.isHoliday =='y'}">是</c:if>
						<c:if test="${overTime.isHoliday =='n'}">否</c:if>
					</td>
				</tr>
			...
			<c:if test="${type=='officeRoom'}">
				...
				<tr><td>使用时间：</td>
					<td><fmt:formatDate value="${officeRoom.startTime}" type="both"/>
					~<fmt:formatDate value="${officeRoom.endTime}" type="both"/></td>
				</tr>
				<tr><td>使用时长：</td>
			...
			<c:if test="${type=='projector'}">
				...
				<tr><td>使用时间：</td>
					<td><fmt:formatDate value="${projector.startTime}" type="both"/>
					~<fmt:formatDate value="${projector.endTime}" type="both"/></td>
				</tr>
				<tr><td>申请理由：</td>
					<td>${projector.reason}</td>
				</tr>
				<tr><td>是否需要转接头：</td>
					<td><c:if test="${projector.adapter =='y'}">是</c:if>
						<c:if test="${projector.adapter =='n'}">否</c:if>
					</td>
				</tr>
			...
			<c:if test="${type=='currency'}">
				...
				<tr>
					...
					~<fmt:formatDate value="${currency.endTime}" type="both"/></td>
				</tr>
				<tr>
					<td>申请事项：</td>
			...
			<c:if test="${detailFlag=='todo'}">
				<tr>
					<td>审批意见：</td>
					<td>
						<textarea rows="2" cols="40" name="approvalResult" id="approvalResult"></textarea>
					</td>
				</tr>
			</c:if>
			</tbody>
			...
			<c:if test="${detailFlag=='todo'}">
				<c:if test="${applyUserId != currentUser }">
					<input type="button" onclick="handleTask('y','${task.id}')" value="同意"/>
					<input type="button" onclick="handleTask('n','${task.id}')" value="驳回"/>
				</c:if>
			</c:if>
	</form>
	...
	function handleTask(flag,taskId){
		...
		$.ajax({
			...
			type:"post",
			data:{"flag":flag,"taskId":taskId,"approvalResult":approvalResult},
			dataType:"json",
			success:function(ret){
				console.log(ret+"-------");
				alert("办理成功");
				$(window.parent.document)[0].location.reload();
