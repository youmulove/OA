...
<body>
	<ul class="nav nav-tabs">
		...
		<li><a href="${ctx}/act/task/historic/">我审批的</a></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${page.list}" var="act">
				<c:set var="task" value="${act.hisProIns}" />
				...
				<tr>
					<td>
						<%-- <a href="${ctx}/act/task/alreadyDetail?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInsId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}">${fns:abbr(not empty vars.map.title ? vars.map.title : task.id, 60)}</a> --%>
					</td>
					<td>
					...
					<td>
						<%-- <a href="${ctx}/act/task/alreadyDetail?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInsId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}">详情</a> --%>
					</td>
				</tr>
			</c:forEach>
		</tbody>
