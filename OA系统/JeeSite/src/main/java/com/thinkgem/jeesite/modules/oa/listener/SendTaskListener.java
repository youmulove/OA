		...
		if (!("".equals(copytoId)) && copytoId != null) {
			...
			if ("leave".equals(type)) {
				...
				sb1.append("开始时间:")
						...
						.append(",结束时间:")
						.append(sdf.format(currency.getEndTime()))
						.append(",用章事宜:").append(currency.getApplyItem())
						.append(",内容:").append(currency.getReason())
						.append(",用章类型:").append(currency.getSelectChapters())
						.append(",用章份数:").append(currency.getChaptersNumber());

				oaNotify.setContent(sb.toString());
				oaNotify.setRemarks(sb1.toString());
			}
			// oaNotify.setId(UUID.randomUUID().toString().replace("-", ""));
			oaNotify.setId(Id.toString().replace("-", ""));
