...
public class OaApplyController extends BaseController {
	...
	public String list(OaApply oaApply, HttpServletRequest request,
			HttpServletResponse response, Model model) {
		Page<OaApply> page = oaApplyService.findPage(new Page<OaApply>(request,
				response), oaApply);
	...
	public String delete(OaApply oaApply, RedirectAttributes redirectAttributes) {
		...
		addMessage(redirectAttributes, "删除用章审批成功");
		return "redirect:" + Global.getAdminPath() + "/oa/oaApply/?repage";
	}

	@RequiresPermissions("oa:oaApply:view")
	@RequestMapping(value = "reportList")
	public String reportList(OaApply oaApply, HttpServletRequest request,
			HttpServletResponse response, Model model) {
		Page<OaApply> page = oaApplyService.findWithNames(new Page<OaApply>(
				request, response), oaApply);
		model.addAttribute("page", page);
	...
	public String exportFile(OaApply oaApply, HttpServletRequest request,
			HttpServletResponse response, RedirectAttributes redirectAttributes) {
		System.out.println("导出用章申请报表开始");
		try {
			String fileName = "用章申请报表" + DateUtils.getDate("yyyyMMddHHmmss")
					+ ".xlsx";
			Page<OaApply> page = oaApplyService.findWithNames(
					new Page<OaApply>(request, response, -1), oaApply);
			System.out.println("page" + page);
			new ExportExcel("用章申请报表", OaApply.class)
					.setDataList(page.getList())
					.writeSelectBrower(response, request, fileName).dispose();
			return null;
		} catch (Exception e) {
			addMessage(redirectAttributes, "导出用章申请报表失败！失败信息：" + e.getMessage());
		}
