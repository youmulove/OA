...
public class OaLeavedController extends BaseController {
	...
	public String list(OaLeaved oaLeaved, HttpServletRequest request,
			HttpServletResponse response, Model model) {
		Page<OaLeaved> page = oaLeavedService.findPage(new Page<OaLeaved>(
				request, response), oaLeaved);
	...
	public String reportList(OaLeaved oaLeaved, HttpServletRequest request,
			HttpServletResponse response, Model model) {
		Page<OaLeaved> page = oaLeavedService.findWithNames(new Page<OaLeaved>(
				request, response), oaLeaved);
		model.addAttribute("page", page);
	...
	public String save(OaLeaved oaLeaved, Model model,
	...
	public String delete(OaLeaved oaLeaved,
			RedirectAttributes redirectAttributes) {
	...
	public String exportFile(OaLeaved oaLeaved, HttpServletRequest request,
			HttpServletResponse response, RedirectAttributes redirectAttributes) {
		try {

			String fileName = "请假报表" + DateUtils.getDate("yyyyMMddHHmmss")
					+ ".xlsx";
			Page<OaLeaved> page = oaLeavedService.findWithNames(
					new Page<OaLeaved>(request, response, -1), oaLeaved);
			System.out.println("page" + page);
			new ExportExcel("请假报表", OaLeaved.class).setDataList(page.getList())
