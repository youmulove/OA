...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/act/oaNotify/form?id=${oaNotify.id}">通知<shiro:hasPermission name="oa:oaNotify:edit">${oaNotify.status eq '1' ? '查看' : not empty oaNotify.id ? '修改' : '添加'}</shiro:hasPermission><shiro:lacksPermission name="oa:oaNotify:edit">查看</shiro:lacksPermission></a></li> --%>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="oaNotify" action="${ctx}/oa/oaNotify/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<input type="text" class="input-xlarge required" value="${oaNotify.title}">
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				...
				<%-- <input type="text" class="input-xxlarge required" value="${oaNotify.content}"/> --%>
				<textarea readonly="readonly" rows="5" cols="300" class="input-xxlarge required">${oaNotify.remarks}</textarea>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
